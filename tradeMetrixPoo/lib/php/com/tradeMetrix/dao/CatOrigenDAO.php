...
class CatOrigenDAO extends DaoAbstract implements InterfaceFuncionesCatalogoGenerico{    
    ...
    public function BusquedaCatalogoApp($filtro=null){
        $this->_query = "SELECT id as idCatalogo,origen as descripcionCatalogo FROM cat_origen
                ".($filtro != "" ? " WHERE ".$filtro : "");
    ...
    public function InsertaCatalogoApp($data){
        $this->_insertField = $this->_contentArrayTabla->CamposCatOrigenInsercion();
        array_shift($this->_insertField);
        $this->_resultSet = $this->insert($this->link, $this->_contentArrayTabla->TablaCatOrigen(), $this->_insertField, substr($this->_contentArrayTabla->TipoCamposCatGenerico(), 1), $data);                
    ...
    public function ActualizarCatalogoApp($data){
        $this->_campos = $this->_contentArrayTabla->CamposCatOrigenActualizacion();
        $this->_clave = $this->_contentArrayTabla->CamposLLaveCatalogoActualizacion();
        $this->_resultSet = $this->update($this->link,$this->_contentArrayTabla->TablaCatOrigen(),$this->_campos,$this->_clave,$this->_contentArrayTabla->TipoCamposCatGenericoActualizacion(),$data);                
        if($this->_resultSet != null || $this->_resultSet != ""){
    ...
    public function EliminarCatalogoApp($data){
        ...
        $this->_resultSet = $this->delete($this->link,$this->_contentArrayTabla->TablaCatOrigen(),$this->_clave,$this->_contentArrayTabla->TipoCamposCatGenericoEliminacion(),$data);
        if($this->_resultSet != null || $this->_resultSet != ""){
            $this->_value=array_pop($data);
            $this->_id = ($this->_value > 0 ? $this->_value : false);    
        }else{

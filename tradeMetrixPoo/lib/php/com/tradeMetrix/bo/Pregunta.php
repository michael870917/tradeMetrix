...
class Pregunta{    
    ...
    public function BuscaPreguntas($tipoSalida=null){
        if(($tipoSalida == "" && count($this->_propiedades) == 0) || ($tipoSalida != "" && count($this->_propiedades) == 0)){
            ...
            $this->_retorno = ($Rs !== false && $tipoSalida == Constantes::OUTPUTSLISTA ? $this->FormaObjetoListaPreguntas($Rs) : false);
        }
        return $this->_retorno;
    }
    ...
    public function BusquedaPreg($value){        
        $Rs=$this->_objetoClassDAO->BusquedaPreguntasApp($value);
    ...
    public function  FormaObjetoListaPreguntas($array){
        ...
        foreach($array as $key => $value){
            ...
            $objMayor->setListOpciones($this->BusquedaOpc($value->idPregunta));
            $read->setNuevoObjetoPregunta($objMayor);
            $read->cargarObjetosPreguntas();
        }        
        $coleccion=$read->getListaObjetosPreguntas();
        return $coleccion;
    }
    ...
    public function  FormaObjetoListaRespuestas($array){        
        ...
        foreach($array as $key => $value){
            ...
            $read->cargarObjetosRespuestas();
        }        
        $coleccion=$read->getListaObjetosRespuestas();
        return $coleccion;
    }
    ...
    public function  FormaObjetoListaOpcion($array){
        ...
        foreach($array as $key => $value){            
            ...
            $read->setNuevoObjetoOpciones($objMayor);
            $read->cargarObjetosOpciones();             
        }        
        $coleccion=$read->getListaObjetosOpciones();
        return $coleccion;
    }
}
...
class LecturaCargas{
    private $_arregloObjetosPregunta = array();
    private $_arregloObjetosRespuesta = array();
    private $_arregloObjetosOpciones = array();
    
    ///////////////////////////////////////////////
    // Opera Bloque Preguntas    
    public function cargarObjetosPreguntas(){
        array_push($this->_arregloObjetosPregunta,$this->nuevoArreglo);
    }
    
    public function getListaObjetosPreguntas(){
        return $this->_arregloObjetosPregunta;
    }
    ...
    ///////////////////////////////////////////////
    // Opera Bloque Respuestas    
    public function cargarObjetosRespuestas(){
        array_push($this->_arregloObjetosRespuesta,$this->nuevoArregloResp);
    }
    
    public function getListaObjetosRespuestas(){
        return $this->_arregloObjetosRespuesta;
    }
    
    public function setNuevoObjetoRespuesta(Respuesta $objetoIncorporar){
        $this->nuevoArregloResp=$objetoIncorporar;
    }
    
    ///////////////////////////////////////////////
    // Opera Bloque Opciones    
    public function cargarObjetosOpciones(){
        array_push($this->_arregloObjetosOpciones,$this->nuevoArregloOpc);
    }
    
    public function getListaObjetosOpciones(){
        return $this->_arregloObjetosOpciones;
    }
    
    public function setNuevoObjetoOpciones(Opcion $objetoIncorporar){
        $this->nuevoArregloOpc=$objetoIncorporar;
    }
}

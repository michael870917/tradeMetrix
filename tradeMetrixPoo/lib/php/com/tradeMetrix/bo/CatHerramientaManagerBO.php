...
include_once $CFG->dataroot.'/lib/php/com/tradeMetrix/dao/CatHerramientaDAO.php';
include_once $CFG->dataroot.'/lib/php/com/tradeMetrix/bo/CatalogoGenerico.php';
include_once $CFG->dataroot.'/lib/php/application/Utilities.php';

class CatHerramientaManager implements ManagerInterface{
    ...
    public function __construct(){
        ...
        $this->_objetoCatHerramientaDAO = new CatHerramientaDAO();
        $this->_objetoCatHerramientaBO = new CatalogoGenerico();
        $this->_utilities = new Utilities();
    }
    
    public function __destruct(){
        unset($this->_objetoAccionesValidacion);
        unset($this->_objetoCatHerramientaDAO);
        unset($this->_objetoCatHerramientaBO);
    ...
    public function BuscaCatalogo($tipoSalida=null){
        if(($tipoSalida == Constantes::OUTPUTSIMPLE && count($this->_propiedades) == 0) || ($tipoSalida == "" && count($this->_propiedades) > 0)){
            ...
            if(($tipoSalida == "" && count($this->_propiedades) == 0) || ($tipoSalida == Constantes::OUTPUTSLISTA && count($this->_propiedades) == 0)){
                $Rs=$this->_objetoCatHerramientaDAO->BusquedaCatalogoApp();
            }elseif($tipoSalida != "" && count($this->_propiedades) > 0){
                $condicional=$this->_objetoAccionesValidacion->Where($this->_propiedades);
                $Rs=$this->_objetoCatHerramientaDAO->BusquedaCatalogoApp($condicional);
            }
    ...
    public function FormaObjetoSimple($array){
        $objManejador= $this->_objetoCatHerramientaBO;
    ...
    public function InsertaCatHerramienta($obj){
        if($obj != "" || $obj != NULL){
            if(is_object($obj)){
                $this->_dataEntry=array($obj->getDescripcionCatalogo());
                $lastId=$this->_objetoCatHerramientaDAO->InsertaCatalogoApp($this->_dataEntry);
    ...
    public function ActualizaCatHerramienta($obj){
        if($obj != "" || $obj != NULL){
            if(is_object($obj)){
                $this->_dataEntry=array($obj->getDescripcionCatalogo(),$obj->getIdCatalogo());
                $this->_value = $this->_objetoCatHerramientaDAO->ActualizarCatalogoApp($this->_dataEntry);
            }else{
                $this->_id=false;
            }
        }else{
    ...
    public function EliminarCatHerramienta($obj){
        if($obj != "" || $obj != NULL){
            if(is_object($obj)){
                $this->_dataEntry=array($obj->getIdCatalogo());
                $this->_value = $this->_objetoCatHerramientaDAO->EliminarCatalogoApp($this->_dataEntry);
            }else{
                $this->_id = false;
            }
        }else{
